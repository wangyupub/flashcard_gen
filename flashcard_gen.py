...
def main(argv):
	...
	try:
		...
		print('flashcard_gen.py -i <inputfile> -o <outputfile>')
		sys.exit(2)
	for opt, arg in opts:
		if opt == '-h':
			print('flashcard_gen.py -i <inputfile> -o <outputfile>')
	...
	if (inputfile == ''):
		inputfile = "word.txt"
	if (outputfile ==''):
		outputfile = inputfile[:inputfile.find(".")] + "_flash_card.txt"
	error = inputfile[:inputfile.find(".")] + "_error.txt"
	generate(inputfile, outputfile, error)


def generate(filename, flashcardfilename, errorfilename):
	fp = open(filename, 'r')
